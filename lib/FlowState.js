export class FlowState {
  ...
  constructor(root, config = {}, hooks = {}) {
    ...
    this.#root.addEventListener('flow-state-get', (e) => {
      ...
      let higherFlow = this.#hasOverwritingFlow(document, this.#root, key);
  ...
  #hasOverwritingFlow(root, target, key) {
    const getParent = (el) => {
      if (el.parentElement) {
  ...
  #updateBindingsForKey(key) {
    ...
    propBindings.forEach(el => {
      ...
      let hasOverwritingFlow = this.#hasOverwritingFlow(this.#root, el, key);
